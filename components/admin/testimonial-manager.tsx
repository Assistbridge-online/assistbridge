...
interface Testimonial {
  id: string; name: string; role: string; country: string | null; quote: string;
  rating: number; avatarSeed: number | null; order: number; active: boolean;
}

const blank: Testimonial = { id: "", name: "", role: "", country: "", quote: "", rating: 5, avatarSeed: null, order: 0, active: true };
...
export function TestimonialManager({ items }: { items: Testimonial[] }) {
  ...
  async function save() {
    if (!editing) return;
    if (!editing.name || !editing.quote) { toast.error("Name and quote required"); return; }
    setLoading(true);
    ...
    return (
      <Card className="p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold">{editing.id ? "Edit" : "New"} testimonial</h2>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div><label className="text-sm font-medium">Country</label><input value={editing.country ?? ""} onChange={(e) => setEditing({ ...editing, country: e.target.value })} className="mt-1 w-full h-10 rounded-lg border border-slate-300 px-3 text-sm" /></div>
          ...
          <div><label className="text-sm font-medium">Rating (1–5)</label><input type="number" min="1" max="5" value={editing.rating} onChange={(e) => setEditing({ ...editing, rating: parseInt(e.target.value) || 5 })} className="mt-1 w-full h-10 rounded-lg border border-slate-300 px-3 text-sm" /></div>
          ...
          <div className="md:col-span-2"><label className="text-sm font-medium">Quote *</label><textarea rows={4} value={editing.quote} onChange={(e) => setEditing({ ...editing, quote: e.target.value })} className="mt-1 w-full rounded-lg border border-slate-300 p-2 text-sm" /></div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-left text-xs font-semibold text-slate-500 uppercase">
              <th className="px-4 py-3">Name</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Quote</th><th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
            {items.map((t) => (
              <tr key={t.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-slate-600 line-clamp-2 max-w-md">{t.quote}</td>
