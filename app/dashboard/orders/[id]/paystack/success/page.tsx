...
import { completePaystack } from "@/lib/actions/checkout";
import { toast } from "sonner";

export const dynamic = "force-dynamic";
export default function PaystackSuccessPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    // Paystack redirects with ?trxref=...&reference=... (or just ?reference=
    // depending on version). We accept both shapes for forward-compat.
    const reference =
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <Card className="p-8 max-w-md w-full text-center">
        ...
        <h1 className="mt-5 text-2xl font-bold text-slate-900">Paystack payment confirmed</h1>
